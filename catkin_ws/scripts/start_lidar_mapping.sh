...
MAPPING_PROFILE="${MAPPING_PROFILE:-baseline}"
case "$MAPPING_PROFILE" in
  baseline|quality) ;;
  *)
    echo "ERROR: MAPPING_PROFILE must be 'baseline' or 'quality' (got '$MAPPING_PROFILE')." >&2
    exit 1
    ;;
esac

DEFAULT_REALSENSE_DEPTH_WIDTH=640
DEFAULT_REALSENSE_DEPTH_HEIGHT=480
DEFAULT_REALSENSE_COLOR_WIDTH=640
DEFAULT_REALSENSE_COLOR_HEIGHT=480
DEFAULT_REALSENSE_DEPTH_FPS=6
DEFAULT_REALSENSE_COLOR_FPS=6
DEFAULT_REALSENSE_FILTERS=""
DEFAULT_CAMERA_MAX_RANGE=5.0
DEFAULT_CAMERA_OUTLIER_FILTER=none
DEFAULT_CAMERA_KEYFRAME_MIN_TRANSLATION=0.0
DEFAULT_CAMERA_KEYFRAME_MIN_ROTATION_DEG=0.0

if [ "$MAPPING_PROFILE" = "quality" ]; then
  # Conservative quality profile: improves D435 local noise without enabling
  # temporal/hole-filling filters or registration before dataset gates approve them.
...
SAVE_CAMERA_DIAGNOSTIC="${SAVE_CAMERA_DIAGNOSTIC:-true}"
SAVE_FUSED_QUALITY_ALIAS="${SAVE_FUSED_QUALITY_ALIAS:-true}"
...
mapping_validate_bool SAVE_CAMERA_DIAGNOSTIC "$SAVE_CAMERA_DIAGNOSTIC"
mapping_validate_bool SAVE_FUSED_QUALITY_ALIAS "$SAVE_FUSED_QUALITY_ALIAS"
...
start_process accumulator roslaunch scout_pointcloud_accumulator accumulate.launch \
  ...
  save_camera_diagnostic:="$SAVE_CAMERA_DIAGNOSTIC" \
  save_fused_quality_alias:="$SAVE_FUSED_QUALITY_ALIAS" \
...
Accumulated PCD outputs:
  ...
  ${PCD_FILE%.pcd}_camera_diagnostic.pcd
